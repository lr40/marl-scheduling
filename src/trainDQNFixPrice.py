...
world_params_dict = {'num_episodes': num_episodes, 'episodeLength': episodeLength, 'numberOfAgents': numberOfAgents, 'numberOfCores': numberOfCores,\
    ...
    'newJobsPerRoundPerAgent': newJobsPerRoundPerAgent, 'rewardMultiplier': rewardMultiplier,'freePrices': False, 'fixPricesList': fixPricesList,'maxVisibleOffers': maxVisibleOffers }
...
RLparamsDict = {'BATCH_SIZE': BATCH_SIZE,'OFFER_GAMMA': OFFER_GAMMA,'ACCEPTOR_GAMMA': ACCEPTOR_GAMMA, 'RUN_START': RUN_START, 'REPLAY_MEMORY_SIZE': REPLAY_MEMORY_SIZE,\
     'RUN_END': RUN_END,'RUN_DECAY': RUN_DECAY, 'TARGET_UPDATE': TARGET_UPDATE, 'RANDOMPOLICY': RANDOMPOLICY, 'IS_DQN': IS_DQN,'freePrices': False,'netZeroOfferReward': netZeroOfferReward}
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        acceptorActions,offerActions \
            = env.getActionForAllAgents(newAcceptorObservationTensors,newOfferObservationTensors)
        ...
        offerNetRewards, acceptorNetRewards, auctioneerReward, agentReward, acceptionQuality, done = env.step(offerActions,acceptorActions,auctioneer_action)
        ...
        if done:
            ...
            for i in range(len(possibleJobPriorities)):
                verweilzeiten = [t.normalisierte_Verweilzeit for t in world.verweilzeiten if (t.Priorität==possibleJobPriorities[i])&(t.Bedienzeit == possibleJobLengths[i])]
                if verweilzeiten != []:
            ...
            averageEpisodicAcceptorRewards.append((accumulatedAcceptorNetReward / episodeLength).numpy().mean())
...
plotFixPricesResult(world,averageEpisodicAcceptorRewards,averageEpisodicOfferRewards,averageEpisodicPrices,\
    averageEpisodicAuctioneerReward,averageEpisodicDwellTimes,meanJobFraction,parameters)
plt.show()
