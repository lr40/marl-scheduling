...
def plotFixPricesResult(argsDict):
    ...
    optimisticAverageAcceptorReward = (
        F(meanJobFraction.numerator, meanJobFraction.denominator + 1)
    ) / params[
        "numberOfAgents"
    ]  # numCores wurde oben und unten rausgekürzt
    roughAverageOfferReward = (
        F(meanJobFraction.numerator + 1, meanJobFraction.denominator + 1)
        * (params["numberOfCores"])
    ) / (params["numberOfAgents"] * params["collectionLength"])
    ...
    for i in range(len(dwellTimes[0])):
        ...
        sub6.plot(
            data,
            "o",
            label="Kind: {} Prio: {}  Length: {}  Ratio: {}".format(
                i,
                params["possibleJobPriorities"][i],
                params["possibleJobLengths"][i],
                round(params["possibleJobPriorities"][i] / params["possibleJobLengths"][i], 2),
            ),
        )
    ...
    if smooth > 1:
        ...
        for i, dataDict in enumerate(dataDicts):
            if value == "dwellTimes":
                ...
                rawData = [
                    float(entry[1]) if entry[1] is not None else np.nan
                    for entry in dataDict[value]
                ]
            else:
                rawData = [
                    float(entry) if entry is not None else np.nan for entry in dataDict[value]
                ]
...
def plotSeparateRuns(dataDicts, value, variable, smooth=100):
    ...
    if smooth > 1:
        ...
        for dataDict in dataDicts:
            highPJobs = [
                float(entry[1]) if entry[1] is not None else np.nan for entry in dataDict[value]
            ]
            label = (
                variable + " {}".format(round(dataDict["params"][variable], 2))
                if variable != None
                else "None"
            )
    ...
    plt.legend(bbox_to_anchor=(1.5, 0), loc="lower left", borderaxespad=0.0, labels=labels1)
...
def plotTwoPrioritiesAverageAndSDOfMultipleRuns(
    dataDicts, value, smooth, bottom1, top1, printInfo, location
):
    ...
    if smooth > 1:
        ...
        for i, dataDict in enumerate(dataDicts):
            ...
            lowPJobs = [
                float(entry[0]) if entry[0] is not None else np.nan for entry in dataDict[value]
            ]
            x = np.asarray(highPJobs)
            z = np.ones(len(x))
            smoothed_x = np.convolve(x, y, "same") / np.convolve(z, y, "same")
    ...
    if printInfo:
        infotext = "".join(
            "{}: {}  ".format(k, str(v)) for (k, v) in dataDicts[0]["params"].items()
        )
...
def plotSeparateRunsWithManualLabels(
    dataDicts, values, ylabel, manualLabels, bottom1, top1, smooth, location, printInfo
):
    plotPath = dataDicts[0]["plotPath"]
...
def plotSeveralMetricsOfMultipleRuns(
    dataDicts, values, ylabel, manualLabels, bottom1, top1, smooth, printInfo, location
):
    plotPath = dataDicts[0]["plotPath"]
...
def plotOneMetricOfMultipleRunsOfMultipleFolders(
    listOfArgsDicts,
    value,
    ylabel,
    manualLabels,
        ...
        if smooth > 1:
            ...
            for i, dataDict in enumerate(dataDicts):
                if value == "dwellTimes":
                    # bezieht sich auf die hochpriorisierten Jobs
                    rawData = [
                        float(entry[jobKind]) if entry[jobKind] is not None else np.nan
                        for entry in dataDict[value]
                    ]
                if value == "agentRew":
                    rawData = [
                        float(statistics.mean(entry)) if entry is not None else np.nan
                        for entry in dataDict[value]
                    ]
...
def plotSeveralJobMetricsOfMultipleRunsOfMultipleFolders(
    listOfArgsDicts,
    value,
    ylabel,
    manualLabels,
    ...
    for jobKind in jobKinds:
        for dataDicts in listOfArgsDicts:
            ...
            data["episode"] = data.index
            data_long = pd.melt(
                data, id_vars=["episode"], value_vars=data.columns[1 : len(dataDicts)]
            )
            sns.set_style("darkgrid")
            sns.lineplot(data=data_long, x="episode", y="value", ci="sd", color=next(colors))
    plt.ylim(bottom=bottom1, top=top1)
    plt.yticks(
        range(bottom1, top1 + 1)
    )  # plt.yticks(list(chain.from_iterable((i, i+0.5) for i in range(bottom1, top1))))
