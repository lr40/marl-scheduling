...
class PPO:
    def __init__(
        ...
        self.policy_old = ActorCritic(
            self.amountInputChannels, self.numberOfActions, numberOfNeurons
        )
...
class FullyAggregatedPPO(PPO):
    def __init__(self, world, env):
        amountInputChannels = (
            (3 + (2 * world.maxAmountOfOffersToOneAgent)) * world.numberOfCores
        ) + ((world.numberOfCores * 2) + (world.collectionLength * 2))
        numberOfActions = ((world.maxAmountOfOffersToOneAgent + 1) ** world.numberOfCores) * (
            (world.numberOfCores + 1) ** world.collectionLength
        )
        numberOfNeurons = 64
        super().__init__(
            world,
...
class AcceptorPPO(PPO):
    def __init__(self, world, env):
        amountInputChannels = 3 + (
            2 * world.maxAmountOfOffersToOneAgent
        )  # + (2*world.maxVisibleOffers)
...
class FreePriceOfferPPO(object):
    def __init__(self, world, env):
        ...
        self.coreChoserNumOfActions = (
            world.numberOfCores + 1
        )  # Die Möglichkeit, ein Angebot nicht zu machen, gibt es.
        self.coreChooser = PPO(
            world,
            self.coreChooserInputAmount,
            self.coreChoserNumOfActions,
            env.LR_ACTOR,
            env.LR_CRITIC,
            env.OFFER_GAMMA,
        ...
        self.priceChooserInputAmount = (
            4  # prio, Länge für den gewählten Kern; prio, Länge für den eigenen Queue-Slot
        )
        self.priceChoserNumOfActions = world.maxSumToOffer + 1
        self.priceChooser = PPO(
            world,
            self.priceChooserInputAmount,
            self.priceChoserNumOfActions,
            env.LR_ACTOR,
            env.LR_CRITIC,
            env.OFFER_GAMMA,
            env.EPS_CLIP,
    ...
    def selectAction(self, coreChooserInput):
        ...
        if coreChooserAction != 0:
            priceChooserInput = torch.cat(
                (
                    coreChooserInput[(coreChooserAction * 2) : (coreChooserAction * 2 + 2)],
                    coreChooserInput[-2:],
                )
            )
            ...
            priceChooserAction = (
                -5
            )  # Beliebiger, erkennbarer Wert, ums beim Debuggen einfacher erkennen zu können
...
class GloballySharedPPO:
    def __init__(
        ...
        self.buffers = [
            [ExperienceBuffer() for _ in range(subLen)] for _ in range(self.world.numberOfAgents)
        ]
        self.subLen = subLen
    ...
    def update(self, randomIndex0, randomIndex1, lastUpdate):
        ...
        old_states = torch.squeeze(
            torch.stack(self.buffers[randomIndex0][randomIndex1].states, dim=0)
        ).detach()
        old_actions = torch.squeeze(
            torch.stack(self.buffers[randomIndex0][randomIndex1].actions, dim=0)
        ).detach()
        old_logprobs = torch.squeeze(
            torch.stack(self.buffers[randomIndex0][randomIndex1].logprobs, dim=0)
        ).detach()
...
class LocallySharedPPO:
    ...
    def update(self, randomIndex, lastUpdate):
        ...
        old_logprobs = torch.squeeze(
            torch.stack(self.buffers[randomIndex].logprobs, dim=0)
        ).detach()
