...
class DQNEntity(nn.Module):
    ...
    def selectAction(self, observationTensor):
        ...
        eps_treshold = self.RUN_END + (self.RUN_START - self.RUN_END) * math.exp(
            -1.0 * self.world.round / self.RUN_DECAY
        )
...
def optimize_model(env, memory, BATCH_SIZE, policy_net, target_net, GAMMA, optimizer):
    ...
    non_final_mask = torch.tensor(
        tuple(map(lambda s: s is not None, batch.next_state)), device="cpu", dtype=torch.bool
    )
    # Hat das Format torch.tensor([bool,bool,...])

    non_final_next_states = torch.cat(
        [torch.tensor(s).unsqueeze(0) for s in batch.next_state if s is not None]
    )

    state_batch = torch.cat([torch.tensor(state).unsqueeze(0) for state in batch.state])
    action_batch = torch.cat(
        [torch.tensor(action).unsqueeze(0).unsqueeze(0) for action in batch.action]
    )
