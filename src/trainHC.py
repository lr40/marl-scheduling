...
for i_episode in range(num_episodes):
    ...
    for t in count():
        acceptorActions, offerActions = env.getActionForAllDividedAgents(
        ...
        totalAgentRewards = [
            (i + j)
            for i, j in zip(
                [sum(agentEntry.squeeze(1)) for agentEntry in offerNetRewards],
                [sum(agentEntry.squeeze(1)) for agentEntry in acceptorNetRewards],
            )
        ]
        ...
        if done:
            ...
            averageEpisodicAceptorRewards.append(
...
plotFixPricesResult(
    world, averageEpisodicAceptorRewards, averageEpisodicOfferRewards, meanJobFraction, parameters
)
plt.show()
