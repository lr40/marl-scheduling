...
def gatherDividedAuctioneerObservation(world):
    ...
    for i in range(world.numberOfCores):
        # (i + 1) ist die Kern-ID
        observationTensor, correspondingOfferIDs = getAuctioneerAcceptorObservationTensorAndIDs(
            world, (i + 1)
        )
...
def getAuctioneerAcceptorObservationTensorAndIDs(world, coreID):
    ...
    coreObservation = [
        {
            ...
            "netReward": (core.job.priority) if ownershipFlag else (-1),
            "remainingLength": (core.job.remainingLength) if ownershipFlag else (-1),
        }
    ]
    offersToThatCoreToThatAgent = [
        {"offeredReward": offer.offeredReward, "necessaryTime": offer.necessaryTime}
        for offer in world.offers
        if (offer.recipientID == 0) & (offer.coreID == coreID)
    ]
    ...
    offersToThatCoreToThatAgent = pad(
        offersToThatCoreToThatAgent,
        world.maxAmountOfOffersToOneAgent,
    ...
    observationTensor = torch.tensor(
        np.concatenate(
            (
                ...
                np.array([list(dicti.values()) for dicti in offersToThatCoreToThatAgent]).reshape(
                    -1
                ),
            )
        ),
...
class Auctioneer(object):
    def __init__(self, world):
        ...
        self.auctioneerID = 0
        self.policy = [
            HardcodedAuctioneerAcceptor(self.world) for _ in range(self.world.numberOfCores)
        ]
        self.ownedCores = []
