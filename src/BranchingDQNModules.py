...
class AgentConfig:
    def __init__(
        ...
        self.epsilon_by_frame = lambda i: self.epsilon_final + (
            self.epsilon_start - self.epsilon_final
        ) * np.exp(-1.0 * i / self.epsilon_decay)
...
class BranchingQNetwork(nn.Module):
    def __init__(self, obs, ac_dim, n):
        ...
        self.model = nn.Sequential(nn.Linear(obs, 128), nn.ReLU(), nn.Linear(128, 128), nn.ReLU())
