...
Verweilzeit = namedtuple(
    "Verweilzeit", ["Prioritaet", "Bedienzeit", "Verweilzeit", "normalisierte_Verweilzeit"]
)
...
class Core(object):
    ...
    def getCoreObservation(self):
        ...
        resultDictionary = {
            ...
            "remainingLength": jobState["remainingLength"],
        }
        return resultDictionary
    ...
    def dispatchNewJobAndReturnOldOne(self, newJob):
        ...
        if (
            self.job.empty == True
        ):  # Diese Fallunterscheidung ist nur der Übersicht halber, damit klar ist, dass auch leere Jobs retourniert werden.
...
class Job(object):
    ...
    def __init__(self, ownerID, priority, initialLength, empty, birthDate, jobKind):
        if empty:
            ...
            self.jobKind = (
                -1
            )  # bleibt das Attribut auf True. Nach Terminierung dieses Jobs, wird es auf True gesetzt und signalisiert damit dem randomisierten Nachfüller,
    ...
    def getJobObservation(self):
        vars1 = vars(self)
        resultDictionary = {
            key: vars1[key] for key in ["priority", "remainingLength", "empty", "wait"]
        }
        return resultDictionary
...
class JobCollection(list):
    ...
    def insertJob(
        self, job
    ):  # Bewirkt, dass der erste freie Platz befüllt wird, aber dass andere Einträge keinesfalls verschoben werden.
    ...
    def removeAndReturnEntry(self, queuePosition):
        ...
        self[queuePosition] = Job(
            0, 0, 0, True, None, None
        )  # Einfügen des leeren Jobs, damit die Liste immer mit Jobs befüllt ist.
        self.numberOfFreeSlots += 1
        return result
...
class Offer(object):
    ...
    def getOfferObservation(self):
        ...
        resultDictionary = {
            key: vars1[key]
            for key in ["offerID", "offererID", "coreID", "priority", "necessaryTime"]
        }
        return resultDictionary
...
class World(object):
    def __init__(self, params):
        ...
        self.accProbabilities = [
            sum(self.probabilities[: (i + 1)]) for i in range(len(self.probabilities))
        ]
        # Stimmt nicht!
        ...
        self.maxAmountOfOffersToOneAgent = (
            self.numberOfAgents
        ) * self.collectionLength  # You do shake hands with yourself to reduce complexity.
        self.maxAmountOfAcceptionsPerTimeStepPerAgent = min(
            self.maxAmountOfOffersToOneAgent, self.numberOfCores
        )
        self.offers = (
            []
        )  # Hier waren früher leere Angebote drin. Die müssen ja aber eigentlich nur Teil der Beobachtung sein.
        ...
        self.acceptedOfferRatios = [
            [[] for _ in range(self.numberOfCores)] for _ in range(self.numberOfAgents)
        ]
        self.auctioneer = Auctioneer(self)
        self.auctioneer.updateOwnedCores(self)

    def resetLiabilityListForACore(
        self, coreID
    ):  # -1, weil Index und ID um eins verschoben sind, +1, weil ja noch der Auktionator dazu kommt
    ...
    def executeAnOffer(self, offerID):
        ...
        if offer.recipientID == core.ownerID:
            ...
            if recipientID != 0:
                recipient.collection.insertJob(
                    oldJob
                )  # Der alte Job wird wieder an die annehmende Partei zurückgegeben. Dass dafür auch Platz ist muss der Zufallsgenerator sicherstellen.
                self.acceptedOfferRatios[recipientID - 1][core.coreID - 1].append(
                    [offer.offeredReward, offer.necessaryTime]
                )
            liabilityListEntry = copy.deepcopy(offer)
            liabilityListEntry.round = self.round
            self.liabilityList[core.coreID - 1].appendleft(
                liabilityListEntry
            )  # -1 von der CoreID;
    ...
    def step1(
        self,
        allOfferNetActionTuples,
        allAcceptorNetsActionTensors,
        ...
        self.executeAgentAcceptions1(
            allAcceptorNetsActionTensors, correspondingOfferIDs
        )  # auskommentieren um die Kein-Trading-Benchmark zu erstellen
        ...
        for core in self.cores:
            if core.job.empty == False:
                ...
                if core.job.remainingLength == 0:
                    ...
                    self.jobTerminationInfo.append(
                        copy.deepcopy((core, ownerID, jobID, generatedReward, self.round + 1))
                    )
                    ...
                    self.verweilzeiten.append(
                        Verweilzeit(
                            core.job.priority,
                            core.job.initialLength,
                            self.round - core.job.birthDate,
                            (self.round - core.job.birthDate - 1) / core.job.initialLength,
                        )
                    )
    ...
    def fillQueuesWithNewRandomJobs(self):
        for agent in self.agents:
            if (
                len(agent.ownedCores) + self.newJobsPerRoundPerAgent
            ) <= agent.collection.numberOfFreeSlots:
    ...
    def createFixPriceOfferObjectsFromActions(self, allOfferNetActionLists):
        ...
        for i, agent in enumerate(self.agents):
            ...
            for j, action in enumerate(actionList):
                ...
                if (
                    (correspondingCore is not None)
                    & (correspondingJob.empty == False)
                    & (correspondingJob.wait == False)
                ):
                    necessaryTime = correspondingJob.remainingLength
                    offerObject = Offer(
                        offeringAgent.agentID,
                        correspondingCore.ownerID,
                        correspondingCore.coreID,
                        slot,
                        correspondingJob.jobID,
                        offeredReward,
                        necessaryTime,
                        False,
                        self.round,
                        correspondingJob.priority,
                    )
