...
def saveCheckpoint(PATH,world):
    ...
    for i, agent in enumerate(world.agents):
        ...
        for j, targetNet in enumerate(agent.target['AcceptorNets']):
            ...
            agentLayerDict['acceptorNet {} optim'.format(j)]= agent.optimizers['acceptorOptimizers'][j].state_dict()
        for j, targetNet in enumerate(agent.target['OfferNets']):
            agentLayerDict['offerNet {}'.format(j)]=targetNet.state_dict()
            agentLayerDict['offerNet {} optim'.format(j)]= agent.optimizers['offerOptimizers'][j].state_dict()
...
def loadCheckpoint(PATH, world):
    ...
    for i,agent in enumerate(world.agents):
        ...
        for j, policyNet in enumerate(agent.policy['AcceptorNets']):
            ...
            policyNet.eval()
            agent.target['AcceptorNets'][j].load_state_dict(agentLayerDict['acceptorNet {}'.format(j)])
            agent.target['AcceptorNets'][j].eval()
            agent.optimizers['acceptorOptimizers'][j].load_state_dict(agentLayerDict['acceptorNet {} optim'.format(j)])
        for j, policyNet in enumerate(agent.policy['OfferNets']):
            ...
            agent.optimizers['acceptorOptimizers'][j].load_state_dict(agentLayerDict['offerNet {} optim'.format(j)])