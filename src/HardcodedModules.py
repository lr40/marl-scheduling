...
def calculateRewardRatio(
    priority, remainingLength
):  # Kommt sowohl in Agent als auch Auctioneer vor. Refactoren in gemeinsame Oberklasse Spieler oder so. Da auch updateCores()- Methode rein
...
class HardcodedAcceptor(object):
    ...
    def selectAction(self, observationTensor):
        ...
        if observationList[0] == 0:
            ...
            listOfOfferdRatios = [
                calculateRewardRatio(priority, necessaryTime)
                for (priority, necessaryTime) in OfferTuples
            ]
            if max(listOfOfferdRatios) > ownRewardRatio:
                acceptionCandidates = [
                    (i, rewardRatio)
                    for (i, rewardRatio) in enumerate(listOfOfferdRatios)
                    if (rewardRatio == max(listOfOfferdRatios))
                ]
...
class HardcodedOfferer(object):
    ...
    def selectAction(self, observationTensor):
        ...
        listOfCoreRatios = [
            calculateRewardRatio(priority, remainingLength)
            for (priority, remainingLength) in CoreStateTuples
        ]
        offerCandidates = [
            (i, rewardRatio)
            for (i, rewardRatio) in enumerate(listOfCoreRatios)
            if (rewardRatio == min(listOfCoreRatios))
        ]
        finalOfferCoreID = random.sample(offerCandidates, 1)[0][
            0
        ]  # Es muss nicht 1 hinzuaddiert werden, weil das schon in world.py geschieht
