...
def getDividedFreePricesReward(env, commercialFreePriceReward):
    ...
    priceChooserRewards = np.array(
        [
            [[0] for slot in range(env.world.collectionLength)]
            for agent in range(env.world.numberOfAgents)
        ],
        dtype=float,
    )
    coreChooserRewards = np.array(
        [
            [[0] for slot in range(env.world.collectionLength)]
            for agent in range(env.world.numberOfAgents)
        ],
    ...
    if commercialFreePriceReward is True:
        for offer in env.world.acceptedOffers:
            ...
            coreChooserReward = (
                offer.prio1
            )  # (offer.prio1 / offer.necessaryTime) - ((env.formerCorePrios[offer.coreID - 1] / env.formerCoreLengths[offer.coreID - 1]) if (env.formerCorePrios[offer.coreID - 1] != -1) else 0)
            priceChooserReward = (
                env.netZeroOfferReward
                if ((offer.prio1 - offer.offeredReward) == 0)
                else (offer.prio1 - offer.offeredReward)
            )
    ...
    acceptorNetRewards = np.array(
        [
            [[0] for core in range(env.world.numberOfCores)]
            for agent in range(env.world.numberOfAgents)
        ]
    )
    ...
    return (
        (coreChooserRewards, priceChooserRewards),
        acceptorNetRewards,
        auctioneerReward,
        agentReward,
    )
