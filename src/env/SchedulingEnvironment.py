...
class SchedulingEnv(gym.Env):
    def __init__(self, world, params):
        ...
        self.correspondingOfferIDs = [[[] for _ in range(world.numberOfCores)]for _ in range(world.numberOfAgents)]
    ...
    def step(self,offerActions,acceptorActions,auctioneer_action):
        self.world.step1(offerActions,acceptorActions,self.correspondingOfferIDs,auctioneer_action, self.auctioneer_correspondingOfferIDs)
        ...
        for agent in self.world.agents:
            acceptorNetObservationTensors,acceptorNetOfferIDs,offerNetObservationTensors = agent.gatherObservations()
        ...
        auctioneer_observation, auctioneer_correspondingOfferIDs = gatherDividedAuctioneerObservation(self.world)
        ...
        return allAcceptorObservationTensors,allOfferObservationTensors,auctioneer_observation, offerRewards, acceptorRewards, auctioneerReward, agentReward, acceptionQualityMean, done
    ...
    def reset(self):
        ...
        return allAcceptorNetObservationTensors,allOfferNetObservationTensors, auctioneer_observation
    ...
    def render(self, mode='human'):
        ...
        for core,ownerID,jobID,generatedReward,round1 in self.world.jobTerminationInfo:
            print("coreID: {}, ownerID: {}, jobID: {}, generatedReward: {}, round: {}".format(core.coreID,ownerID,jobID,generatedReward,round1))
        print("Accepted Offers:")
        for offer in self.world.acceptedOffers:
            print("coreID: {}, offererID: {}, recipientID: {}, jobID: {}, offeredReward {}, slot: {}"\
                .format(offer.coreID,offer.offererID,offer.recipientID,offer.jobID,offer.offeredReward,offer.queuePosition))
    
    def getActionForAllAgents(self,nestedAcceptorNetObservationTensors,nestedOfferNetObservationTensors):
        nestedAcceptorNetActionTensors = list([[-5 for core in range(self.world.numberOfCores)] for agent in range(self.world.numberOfAgents)])
        nestedOfferNetActionTensors = list([[-5 for slot in range(self.world.collectionLength)] for agent in range(self.world.numberOfAgents)])
        for i, agent in enumerate(self.world.agents):
            offerNetActions, acceptorNetActions = agent.getActions(nestedOfferNetObservationTensors[i],nestedAcceptorNetObservationTensors[i])
    ...
    def calculateAverageAcceptionQuality(self):
        ...
        for offer in nonAuctioneerOffers:
            acceptionQuality = (offer.offeredReward / offer.necessaryTime) - ((self.formerCorePrios[offer.coreID - 1] / self.formerCoreLengths[offer.coreID - 1]) if (self.formerCorePrios[offer.coreID - 1] != -1) else 0)
            acceptionQuality *= 10
...
class PPOAggregatedFixPriceEnv(PPOSchedulingEnv):
    def __init__(self,world,params):
        super().__init__(world,params)
        self.world.agents = [AggregatedFixPricePPOAgent(self.world,self) for _ in range(self.world.numberOfAgents)]
...
class PPOFullyAggregatedFixPriceEnv(PPOSchedulingEnv):
    def __init__(self,world,params):
        super().__init__(world,params)
        self.world.agents = [FullyAggregatedFixPricePPOAgent(self.world,self) for _ in range(self.world.numberOfAgents)]
...
class PPODividedFreePriceEnv(PPOSchedulingEnv):
    def __init__(self,world, params, commercialFreePriceReward):
        super().__init__(world,params)
        self.world.agents = [DividedFreePricePPOAgent(self.world,self) for _ in range(self.world.numberOfAgents)]
...
class PPODividedFixedPriceEnv(PPOSchedulingEnv):
    def __init__(self,world, params):
        super().__init__(world,params)
        self.world.agents = [DividedFixedPricePPOAgent(self.world,self) for _ in range(self.world.numberOfAgents)]
...
class GloballySharedParamsDividedFixedPriceEnv(PPOSchedulingEnv):
    def __init__(self, world,params):
        ...
        self.world.agents = [DividedFixedPriceGloballySharedPPOAgent(self.world,self) for _ in range(self.world.numberOfAgents)]
...
class LocallySharedParamsDividedFixedPriceEnv(PPOSchedulingEnv):
    def __init__(self, world,params):
        super().__init__(world,params)
        self.world.agents = [DividedFixedPriceLocallySharedPPOAgent(self.world,self) for _ in range(self.world.numberOfAgents)]
    ...
    def updateAcceptorMemoriesAndOptimize(self,acceptorActions,acceptorNetRewards):
        for i, agent in enumerate(self.world.agents):
            ...
            for j in range(self.world.numberOfCores):
                ...
                targetNet = targetNets[j]
                optimizer = optimizers[j]          
                optimize_model(self,memory,self.BATCH_SIZE,policyNet,targetNet,self.ACCEPTOR_GAMMA,optimizer)

class DQNDividedFixedPricesEnv(DQNSchedulingEnv):
    def __init__(self,world,params):
        super().__init__(world,params)
        self.world.agents = [DividedFixPriceDQNAgent(self.world,self) for _ in range(self.world.numberOfAgents)]
...
class HardcodedFixPriceEnvironment(SchedulingEnv):
    def __init__(self,world, params):
        super().__init__(world,params)
        self.world.agents = [DividedHardcodedAgent(self.world) for _ in range(self.world.numberOfAgents)] 
